...
export function useAudio() {
  ...
  const exciterBus = useRef<GainNode | null>(null);
  const modalBus = useRef<GainNode | null>(null);
  const obj1Bus = useRef<GainNode | null>(null);
  const obj2Bus = useRef<GainNode | null>(null);
  const lowCutHPF = useRef<BiquadFilterNode | null>(null);
  const noiseBuffer = useRef<AudioBuffer | null>(null);
  const filterBank = useRef<ModalNode[]>([]);
  const obj1Bank = useRef<CombNode[]>([]);
  const obj2Bank = useRef<CombNode[]>([]);
  const micFilterBank = useRef<ModalNode[]>([]);
  const micObj1Bank = useRef<CombNode[]>([]);
  const micObj2Bank = useRef<CombNode[]>([]);
  const distShaper = useRef<WaveShaperNode | null>(null);
  const currentPitchRef = useRef(220);

  const micStream = useRef<MediaStream | null>(null);
  const micSource = useRef<MediaStreamAudioSourceNode | null>(null);
  const micGain = useRef<GainNode | null>(null);
  const micHPF = useRef<BiquadFilterNode | null>(null);
  const micHPF2 = useRef<BiquadFilterNode | null>(null);
  const micPreGain = useRef<GainNode | null>(null);
  const micAnalyser = useRef<AnalyserNode | null>(null);
  const micExciterGain = useRef<GainNode | null>(null);
  const vuFrameRef = useRef<number>(0);
  const envFrameRef = useRef<number>(0);
  const micEnvValueRef = useRef<number>(0);
  const micResonatorBus = useRef<GainNode | null>(null);

  const compressor = useRef<DynamicsCompressorNode | null>(null);
  const dlyNode = useRef<DelayNode | null>(null);
  const dlyFeedbackGain = useRef<GainNode | null>(null);
  const dlyWetGain = useRef<GainNode | null>(null);
  const dlyDryGain = useRef<GainNode | null>(null);
  const reverbConv = useRef<ConvolverNode | null>(null);
  const reverbWetGain = useRef<GainNode | null>(null);
  const reverbDryGain = useRef<GainNode | null>(null);
  const eqLowShelf = useRef<BiquadFilterNode | null>(null);
  const eqMidPeak = useRef<BiquadFilterNode | null>(null);
  const eqHighShelf = useRef<BiquadFilterNode | null>(null);

  const currentRevSize = useRef<number>(-1);
  const currentRevDecay = useRef<number>(-1);

  const buildImpulse = useCallback((audioCtx: AudioContext, duration: number, decay: number) => {
  ...
  const makeDistCurve = useCallback((amount: number, drive: number, tone: number, distType: string) => {
    const samples = 8192;
    ...
    for (let i = 0; i < samples; i++) {
      ...
      switch (distType) {
        ...
        case 'Sine':
          curve[i] = Math.sin(x * k * 0.5 * Math.PI);
          break;
        case 'Scream': {
          const preEmp = x * k * 0.6 + x * x * x * k * 0.3;
          curve[i] = Math.max(-1, Math.min(1, preEmp));
          break;
        }
        case 'Dist': {
          const pos = x >= 0 ? Math.min(1, x * k * 0.4) : Math.max(-1, x * k * 0.35);
          const asym = pos - pos * pos * pos * 0.33 + (x < 0 ? 0.04 : 0);
          curve[i] = Math.max(-1, Math.min(1, asym * 1.8));
          break;
        }
        case 'Ring': {
          const phase = x * k * 0.8;
          curve[i] = Math.tanh(Math.sin(phase * Math.PI * 2) * (1 + Math.abs(x)));
          break;
        }
        case 'S/H': {
          const steps = Math.max(2, Math.round(4 + (1 - amount) * 28));
  ...
  const applyTuning = useCallback((baseFreq: number, p: AudioParams) => {
    ...
    // Global pitch offset: center=0.5 means no change, range ±24 semitones
    ...
    if (lowCutHPF.current) {
      lowCutHPF.current.frequency.setTargetAtTime(20 + (p.lowCut ?? 0) * 800, now, T);
    }
    ...
    // Rebuild reverb IR when revSize or revDecay changes significantly
    ...
    filterBank.current.forEach((node, i) => {
      const mult = p[`modalFreq${i}`] as number ?? 1;
      ...
      const partialGain = (p[`modal${i}`] as number ?? 1);
    ...
    obj1Bank.current.forEach((node, i) => {
      const mult = p[`obj1Freq${i}`] as number ?? 1;
      ...
      const partialGain = (p[`obj1_${i}`] as number ?? 0.5);
    ...
    obj2Bank.current.forEach((node, i) => {
      const mult = p[`obj2Freq${i}`] as number ?? 1;
      ...
      const fbLevel = Math.min(0.9985, Math.pow(0.001, 1 / cyclesTotal));
      node.fb.gain.setTargetAtTime(fbLevel, now, T);
      const partialGain = (p[`obj2_${i}`] as number ?? 0.5);
      node.out.gain.setTargetAtTime(partialGain * 0.22, now, T);
    ...
    micObj1Bank.current.forEach((node, i) => {
      ...
      node.fb.gain.setTargetAtTime(fbLevel, now, T);
      node.out.gain.setTargetAtTime((p[`obj1_${i}`] as number ?? 0.5) * 0.22, now, T);
      const panSpread = (i % 2 === 0 ? 1 : -1) * (0.15 + i * 0.06) * p.widthObj * p.panObj1;
      node.panner.pan.setTargetAtTime(Math.max(-1, Math.min(1, panSpread)), now, T);
    });
    ...
    micObj2Bank.current.forEach((node, i) => {
      ...
      const fbLevel = Math.min(0.95, Math.pow(0.001, 1 / cyclesTotal) * micFbScale);
      node.fb.gain.setTargetAtTime(fbLevel, now, T);
      node.out.gain.setTargetAtTime((p[`obj2_${i}`] as number ?? 0.5) * 0.22, now, T);
      const panSpread = (i % 2 === 0 ? -1 : 1) * (0.18 + i * 0.07) * p.widthObj * p.panObj2;
      node.panner.pan.setTargetAtTime(Math.max(-1, Math.min(1, panSpread)), now, T);
    });
  ...
  const startVUMeter = useCallback(() => {
    ...
    const tick = () => {
      ...
      setInputLevel(Math.min(1, prevEnv * 14));
  ...
  const connectMicToSynth = useCallback((audioCtx: AudioContext, stream: MediaStream, params: AudioParams) => {
    if (micSource.current) { try { micSource.current.disconnect(); } catch {} }
  ...
  const requestMic = useCallback(async (params: AudioParams) => {
  ...
  }, [micState, connectMicToSynth]);

  const releaseMic = useCallback(() => {
    stopVUMeter();
    if (micSource.current) { try { micSource.current.disconnect(); } catch {} micSource.current = null; }
  ...
  const initAudio = useCallback(async (params: AudioParams) => {
    if (ctx.current) return;
    ...
    delay.connect(dlyFb); dlyFb.connect(delay);
    delay.connect(dlyWet);
  ...
  }, [applyTuning, buildImpulse]);

  const triggerNote = useCallback((freq: number, params: AudioParams) => {
    if (!ctx.current) return;
